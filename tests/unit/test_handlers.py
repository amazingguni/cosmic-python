...
class FakeRepository(repository.AbstractProductRepository):
    ...
    def _get_by_batch_reference(self, batch_reference):
        return next((p for p in self._products for b in p.batches if b.reference == batch_reference), None)
...
class TestAddBatch:
    def test_for_new_product(self):
        ...
        messagebus.handle(commands.CreateBatch(
            'b1', 'CRUNCHY-ARMCHAIR', 100, None), uow)
...
class TestAllocate:
    def test_allocation(self):
        uow = FakeUnitOfWork()
        messagebus.handle(commands.CreateBatch(
            'batch1', 'COMPLICATED-LAMP', 100, None), uow)
    ...
    def test_errors_for_invalid_sku(self):
        ...
        with pytest.raises(handlers.InvalidSku, match='Invalid sku NONEXISTENTSKU'):
            messagebus.handle(commands.Allocate(
                'o1', 'NONEXISTENTSKU', 10), uow)
